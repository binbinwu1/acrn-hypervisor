	...
	if (result == 0) {
		...
		for (i = 1U; i <= limit; i++) {
			...
			case 0x12U:
				if (is_vm_sgx_supported(vm->vm_id)) {
					uint32_t sid;
					struct virt_epc_sections* secs = get_vm_epc(vm->vm_id);
					...
					for (sid = 0U; sid < secs->count; sid++) {
						entry.leaf = i;
						entry.subleaf = sid + CPUID_SGX_EPC_SUBLEAF_BASE;
						entry.flags = CPUID_CHECK_SUBLEAF;
						entry.eax = CPUID_SGX_EPC_TYPE_VALID |
							    (secs->sections[sid].gpa_base & CPUID_SGX_EPC_VAL_LOW_MASK);
						entry.ebx = (secs->sections[sid].gpa_base >> 32U) &
							    CPUID_SGX_EPC_VAL_HIGH_MASK;
						entry.ecx = secs->sections[sid].size & CPUID_SGX_EPC_VAL_LOW_MASK;
						entry.edx = (secs->sections[sid].size >> 32U) &
							    CPUID_SGX_EPC_VAL_HIGH_MASK;
						result = set_vcpuid_entry(vm, &entry);
					}
				}
