...
static uint16_t alloc_irtes(struct dmar_drhd_rt *dmar_unit, uint16_t num)
{
	uint16_t irte_idx;
	uint64_t test_mask = (1UL << num) - 1U;

	ASSERT((bitmap_weight(num) == 1U) && (num <= 32U));

	spinlock_obtain(&dmar_unit->lock);
	for(irte_idx = 0U; irte_idx < CONFIG_MAX_IR_ENTRIES; irte_idx+= num) {
		if ((dmar_unit->irte_alloc_bitmap[irte_idx >> 6U] & (test_mask << (irte_idx & 0x3FU))) == 0UL) {
			dmar_unit->irte_alloc_bitmap[irte_idx >> 6U] |= test_mask << (irte_idx & 0x3FU);
			break;
		}
	}
	spinlock_release(&dmar_unit->lock);
...
static bool is_irte_reserved(struct dmar_drhd_rt *dmar_unit, uint16_t index)
{
...
int32_t dmar_reserve_irte(const struct intr_source *intr_src, uint16_t num, uint16_t *start_id)
{
	...
	if (is_dmar_unit_valid(dmar_unit, sid)) {
		*start_id = (int32_t) alloc_irtes(dmar_unit, num);
